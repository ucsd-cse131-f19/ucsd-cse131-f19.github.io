...
let parse_def sexp =
  match sexp with
    | List([Atom("def");(List([Atom(name);Atom(argname)]));body]) ->
      DFun(name, argname, sexp_to_expr body)
...
let rec find_def (defs : def list) (x : string) : def option =
  match defs with
    | [] -> None
    | (DFun(fname, param, body) as d)::rest ->
...
let compile_def (DFun(name, param, body)) defs =
